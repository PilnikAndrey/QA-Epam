...
public class GamePage {
    ...
    private Label game = new Label(By.xpath("//div[contains(text(),'\t\t\t\t\t\t\tFree To Play\t\t\t\t\t\t') or contains(text(),' \t\t\t\t\t\t\tFree\t\t\t\t\t\t') ]"),"game");
    private Label genre = new Label(By.xpath("//div[@class='details_block']//a[contains(text(),'Action')]"),"genre");
    ...
    public boolean isGtaGame() {
        return gameButton.getText().toUpperCase().equals("Grand Theft Auto V".toUpperCase());
    }

    public boolean isFreeGame() {
        return game.isDisplayed();
    }
